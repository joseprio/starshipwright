...
type OutlineFunc = (shipRandomizer: Randomizer, factionRandomizer: Randomizer, w: number, h: number, hw: number, size: number, csx: CanvasRenderingContext2D) => void;

//Each outline function takes a single argument 'lp' denoting the ship to draw the outline for.
export const outlines: Array<OutlineFunc> = [
  // 0: Joined rectangles.
  function (shipRandomizer: Randomizer, factionRandomizer: Randomizer, w: number, h: number, hw: number, size: number, csx: CanvasRenderingContext2D) {
  ...
  function (shipRandomizer: Randomizer, factionRandomizer: Randomizer, w: number, h: number, hw: number, size: number, csx: CanvasRenderingContext2D) {
    ...
    const initialcount = Math.floor(
      (h - 2 * CANVAS_SHIP_EDGE) / (initialwidth * 2)
    );
    ...
    for (let i = initialcount; i < circlecount; i++) {
      const base =
        circles[
          Math.max(shipRandomizer.si(0, circles.length - 1), shipRandomizer.si(0, circles.length - 1))
        ];
      ...
      ncr = Math.min(
        ...
        w - CANVAS_SHIP_EDGE - lv[0],
        lv[1] - CANVAS_SHIP_EDGE,
        h - CANVAS_SHIP_EDGE - lv[1]
      );
