...
;// ./src/outline/rectangles.ts

function rectangles_generateOutline(layoutSeed, forceSize) {
    const layoutRNG = createPRNGGenerator(layoutSeed);
    const size = forceSize || numberBetween(layoutRNG(), 2.5, 7) ** 3;
    const wratio = numberBetween(layoutRNG(), 0.5, 1.3);
    const hratio = numberBetween(layoutRNG(), 0.7, 1.7);
...
;// ./src/outline/circles.ts

//Size of the component grid
const circles_COMPONENT_GRID_SIZE = 6;
function circles_generateOutline(layoutSeed, forceSize) {
    const layoutRNG = createPRNGGenerator(layoutSeed);
    const size = forceSize || numberBetween(layoutRNG(), 2.5, 7) ** 3;
    const wratio = numberBetween(layoutRNG(), 0.5, 1.3);
    const hratio = numberBetween(layoutRNG(), 0.7, 1.7);
...
;// ./src/outline/lines.ts


//Size of the component grid
const lines_COMPONENT_GRID_SIZE = 6;
function lines_generateOutline(layoutSeed, forceSize) {
    const layoutRNG = createPRNGGenerator(layoutSeed);
    const size = forceSize || numberBetween(layoutRNG(), 2.5, 7) ** 3;
    const wratio = numberBetween(layoutRNG(), 0.5, 1.3);
    const hratio = numberBetween(layoutRNG(), 0.7, 1.7);
    ...
    const basefatness = lines_COMPONENT_GRID_SIZE / size + layoutOutline2BaseFatness;
    const pointcount = Math.max(3, Math.ceil((numberBetween(layoutRNG(), 0.05, 0.1) / basefatness) * size ** 0.5));
    for (let npi = 1; npi < pointcount; npi++) {
        let np = points[npi];
        if (!np) {
...
function generateNextShip() {
  ...
  if (document.getElementById("classic").checked) {
    ...
    generateOutline = rectangles_generateOutline;
  } else if (document.getElementById("circles").checked) {
    generateOutline = circles_generateOutline;
  } else if (document.getElementById("lines").checked) {
    generateOutline = lines_generateOutline;
  }
